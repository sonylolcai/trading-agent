...
def build_decision_stance_guidance(stance: str | None) -> str:
    ...
    common_rules = (
        ...
        "- **§9.0 计划型限价**：宽通道/区间/通道靠边界、或顺势回撤到结构位时，"
        "即使 K1 为 doji/弱棒、尚无已收盘信号棒，§9.0 也可判「是」（计划型入场），"
        "signal_bar.quality 可为 weak/invalid；不要一律 §9.0=否 后观望。\n"
    ...
    if normalized == "conservative":
        profile = (
            ...
            "- **§9.0 计划型限价（边界例外）**：宽通道/区间/通道靠边界时，"
            "即使 direction=neutral、K1 为 doji/弱棒/无信号棒，仍应先评估边界限价三价；"
            "§9.0 可判「是」并输出限价单（非一律观望）。中部位置或无结构锚点时才默认 wait。\n"
        ...
        profile = (
            ...
            "- §9.0 计划型限价（优先于默认观望）：当 cycle_position 为 broad_channel / "
            "trading_range / normal_channel / trending_tr，且价格靠近支撑/阻力/通道边界，"
            "或顺势回撤/反弹到结构位可挂限价时——**即使当前无强信号棒、K1 为 doji/弱棒**，"
            "§9.0 应判「是」，reason 写明「计划型限价，等待回撤/反弹到位」；"
            "signal_bar.bar 可为 null、quality=invalid/weak；entry_bar 设 pending。"
            "**禁止**因「等下一根 K 确认」而将 §9.0 判否后直接不下单——应先评估限价三价与 10.3。\n"
            "- §9 入场：除典型信号外，若结构与阶段一 direction/cycle_position 一致，"
            "允许「次优但可执行」的二类 setup（须在 reason 中写明为何仍值得做）。\n"
            "- 宽通道/区间边界：优先限价单路径；weak 信号棒配合 tr_boundary/L1/H1 等 pattern "
            "仍可下单，须在 §9/reasoning 说明接受的瑕疵。\n"
